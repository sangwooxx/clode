const PLANNING_STORAGE_KEY = "agentPlanningRegistryV1";

const planningState = window.__agentPlanningState || {
  initialized: false,
  selectedDate: "",
  search: "",
  calendarCursor: "",
...
window.__agentPlanningState = planningState;
...
function loadPlanningStore() {
  const parsed = window.AgentDataAccess?.legacy
    ? window.AgentDataAccess.legacy.read(PLANNING_STORAGE_KEY, null)
    : null;
...
function savePlanningStore(store) {
  if (window.AgentDataAccess?.legacy) {
    window.AgentDataAccess.legacy.write(PLANNING_STORAGE_KEY, store, { eventName: "planning-registry-updated" });
    return;
  }
...
function renderPlanningAssignments() {
  ...
  target.innerHTML = `
    <table class="entity-table module-table planning-table">
      <thead>
        <tr>
          <th>Lp.</th>
          <th>Pracownik</th>
          <th>Stanowisko</th>
          <th>Status na dzień</th>
          <th>Kontrakt</th>
          <th>Uwagi</th>
          <th>Akcja</th>
        </tr>
      ...
      <div class="planning-contracts-shell">
        ...
        <table class="entity-table module-table">
          ...
          <tbody>
            ${rows.map((row) => `
              <tr>
                <td>${planningEscape(row.contract.contract_number || "-")}</td>
                <td>${planningEscape(row.contract.name)}</td>
                <td>${planningEscape(row.employees.length ? "Obsadzony" : "Brak obsady")}</td>
                <td>${planningEscape(String(row.employees.length))}</td>
                <td>${planningEscape(row.employees.length ? row.employees.join(", ") : "-")}</td>
              </tr>
            `).join("")}
          </tbody>
        </table>
