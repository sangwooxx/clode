...
export function VacationsRequestPanel({
  ...
  const content = (
    <form className="vacations-form" onSubmit={onSubmitRequest}>
      ...
      {!canWrite ? <p className="status-message">Masz dostęp tylko do podglądu wniosków urlopowych.</p> : null}
      ...
      <div className="vacations-form__actions">
        ...
        {canWrite ? (
          <ActionButton type="submit" disabled={isSubmittingRequest || editingEmployeeNeedsManualResolution}>
            {isSubmittingRequest ? "Zapisywanie..." : editingRequest ? "Zapisz zmiany" : "Dodaj nieobecność"}
          </ActionButton>
        ) : null}
      </div>
    </form>
  ...
  return <Panel title={editingRequest ? "Edytuj wniosek" : "Nowy wniosek / nieobecność"}>{content}</Panel>;
}
