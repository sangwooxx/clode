...
from agent_backend.auth.passwords import hash_password, looks_like_password_hash
from agent_backend.auth.rbac import effective_permissions, normalize_role
from agent_backend.auth.sessions import utc_now_iso
...
class UserService:
    ...
    def ensure_bootstrap_users(self) -> None:
        ...
        username = str(os.getenv("AGENT_BOOTSTRAP_ADMIN_USERNAME", "")).strip().lower()
        password = str(os.getenv("AGENT_BOOTSTRAP_ADMIN_PASSWORD", "")).strip()
        display_name = str(os.getenv("AGENT_BOOTSTRAP_ADMIN_NAME", "Administrator")).strip()
        email = str(os.getenv("AGENT_BOOTSTRAP_ADMIN_EMAIL", "")).strip().lower()
