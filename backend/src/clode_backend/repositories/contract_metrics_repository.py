...
class ContractMetricsRepository(RepositoryBase):
    def _run_with_connection(self, connection, callback):
        if connection is not None:
            return callback(connection)
        with self.connect() as owned_connection:
            return callback(owned_connection)

    def calculate_contract_metrics(self, contract_id: str, time_range: dict[str, Any], connection=None) -> dict[str, Any]:
        selector = contract_id or "unassigned"
        def calculate(active_connection):
            revenue_total = self._sales_total(selector, time_range, active_connection)
            invoice_cost_by_category = self._invoice_cost_by_category(selector, time_range, active_connection)
            labor = self._labor_totals(selector, time_range, active_connection)
            labor_cost = labor["cost_total"]
            labor_hours_total = labor["hours_total"]
            cost_invoice_count = self._invoice_count(selector, time_range, "cost", active_connection)
            sales_invoice_count = self._invoice_count(selector, time_range, "sales", active_connection)
        ...
        return self._run_with_connection(connection, calculate)

    def calculate_global_metrics(self, time_range: dict[str, Any], connection=None) -> dict[str, Any]:
        def calculate(active_connection):
            assigned = self.calculate_contract_metrics("__assigned__", time_range, active_connection)
            unassigned = self.calculate_contract_metrics("unassigned", time_range, active_connection)
            totals = {
    ...
    def list_contract_monthly_breakdown(self, contract_id: str, time_range: dict[str, Any], connection=None) -> list[dict[str, Any]]:
        selector = contract_id or "unassigned"

        def calculate(active_connection):
            invoice_rows = self._invoice_monthly_breakdown(selector, time_range, active_connection)
            labor_rows = self._labor_monthly_breakdown(selector, time_range, active_connection)
    ...
    def list_unassigned_invoices(self, time_range: dict[str, Any], connection=None) -> list[dict[str, Any]]:
        params: list[Any] = []
    ...
    def list_unmatched_hours(self, time_range: dict[str, Any], connection=None) -> list[dict[str, Any]]:
        params: list[Any] = []
    ...
    def _invoice_cost_by_category(self, contract_id: str, time_range: dict[str, Any], connection=None) -> dict[str, float]:
        params: list[Any] = []
        contract_clause = _contract_selector("contract_id", contract_id, params)
        time_clause = _invoice_time_where(time_range, params)
    ...
    def _labor_totals(self, contract_id: str, time_range: dict[str, Any], connection=None) -> dict[str, float]:
        params: list[Any] = []
    ...
    def _invoice_count(self, contract_id: str, time_range: dict[str, Any], invoice_type: str, connection=None) -> int:
        params: list[Any] = []
        contract_clause = _contract_selector("contract_id", contract_id, params)
        time_clause = _invoice_time_where(time_range, params)
    ...
    def _invoice_monthly_breakdown(self, contract_id: str, time_range: dict[str, Any], connection=None) -> list[dict[str, Any]]:
        params: list[Any] = []
        contract_clause = _contract_selector("contract_id", contract_id, params)
        time_clause = _invoice_time_where(time_range, params)
    ...
    def _labor_monthly_breakdown(self, contract_id: str, time_range: dict[str, Any], connection=None) -> list[dict[str, Any]]:
        params: list[Any] = []
        contract_clause = _contract_selector("te.contract_id", contract_id, params)
        time_clause = _hours_time_where(time_range, params)
