...
from clode_backend.services.user_service import UserService, UserServiceError
...
def route_request(
    ...
    session_token = read_session_token_from_headers(
        ...
        handler.headers.get(SESSION_HEADER_NAME) or handler.headers.get(LEGACY_SESSION_HEADER_NAME),
    )
